...
cropped = cropByLd(xfm, c.ld_lim, f.fft_scale);
% bound = boundShades(cropped, c.mag_lims);
mid_vert = round(size(cropped, 1) / 2);
mid_horiz = round(size(cropped, 2) / 2);
r_axis = ((mid_horiz:size(cropped, 2)) - mid_horiz) / f.fft_scale;
