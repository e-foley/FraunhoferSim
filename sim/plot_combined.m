...
function [ logdiff ] = plot_combined(ppmat, ppmat_rel, fft_scale, mag_lims, LD_lim, nominal_plot_size, colors, figure_num, figure_title, filename_string)
...
ppmat = single(ppmat);
ppmat_rel = single(ppmat_rel);

ld_spans = (size(ppmat) / fft_scale);
ld_bounds = (ld_spans ./ 2)' * [-1 1]; % the range of lambda/D shown over whole image

logdiff = cat(3, log10(ppmat), -Inf*zeros(size(ppmat)), log10(ppmat_rel));
